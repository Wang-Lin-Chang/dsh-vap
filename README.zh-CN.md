...
- **小规模信任域**：按单机共享文件系统设计（可扩展到共享盘 / HTTP 网关）。中继路径已在公网实测
  （Tailscale 加密隧道）：跨 NAT 18/18 送达并验签通过（见 `phase6/DEPLOYMENT.md`）、中继吞吐
  502.5 封/秒（见 `phase7/vap-punch-routes.md`）；裸 IP 直连路径仍未实测。
