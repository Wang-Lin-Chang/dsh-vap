...
- **活性三层修复（TLC 驱动）**：① baseline/最高-QC 扩展检查（消除跳跃投票）→ ② Certified(parent) 投票前置（消除投未认证后代）→ ③ view-change/new-view 协议（签名绑定 + 聚合最高 QC 作锚）。TLC 机器验证：7 条安全不变式完整穷举 PASS；**稳定 leader + GST 下 EventuallyCommit PASS（62,064 状态）**；完全对抗模型 FAIL（完全异步对抗场景，与 FLP 定理边界一致；实现侧 detectEquivocation 除名兜底）。全程记录见 `tlaplus/TLC-RESULTS.md` 与 PROOFS.md §4.1/§5 开放问题第 4 条。
